...
def policy_evaluation(policy, n_nodes, travel_matrix, edge_prob, gamma=0.9, tol=1e-3):
    ...
    for _ in range(500):
        ...
        for s in range(n_nodes):
            a = policy[s]
            if a == s: continue
            V[s] = -travel_matrix[s,a]/edge_prob[s,a] + gamma*V_prev[a]
        if np.max(np.abs(V - V_prev)) < tol:
            break
    return V

def policy_iteration(n_nodes, travel_matrix, edge_prob, gamma=0.9):
    policy = np.arange(n_nodes, dtype=np.int32)
    for _ in range(100):
        V = policy_evaluation(policy, n_nodes, travel_matrix, edge_prob, gamma)
        stable = True
        for s in range(n_nodes):
            best_a = np.argmax([-travel_matrix[s,a]/edge_prob[s,a] + gamma*V[a]
...
def simulate_vehicle(vid, cluster_nodes, node2idx, idx2node, depot, travel_matrix, edge_prob,
    ...
    policy, _ = policy_iteration(n_nodes, travel_matrix, edge_prob)
