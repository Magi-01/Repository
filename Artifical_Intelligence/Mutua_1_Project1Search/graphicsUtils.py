...
def keys_pressed(d_o_e=tkinter.Tk().dooneevent, d_w=0):
    root = tkinter.Tk()  # Create a root window
    root.update()  # Update the root window to process events
    d_o_e(d_w)  # Process one event without waiting

    if _got_release:
        d_o_e(d_w)  # Process another event if a release event occurred

    return _keysdown.keys()  # Return a list of currently pressed keys
