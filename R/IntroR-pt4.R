...
getwd()
setwd("C:/Users/mutua/Documents/Repository/R/") # o Session -> 
                                    # Set Working Directory or Files tab -> More 
...
Cipolle <- read.table("DA-dataset/DA-dataset/cipolle.dat",
                      header = T, stringsAsFactors = T)
Windmill <- read.table("DA-dataset/DA-dataset/windmill.txt",
                       header = T, dec = ",")
Machine <- read.csv("DA-dataset/DA-dataset/macchine/macchine.data",
                    header = F, na.strings = "?")
...
is.numeric(Food$Gender) #accedendo ad una singola colonna è possibile
                        # trattarla come vettore 
...
#selezioni    amo il genere dei soggetti con sport==1 e vitamine==1
...
#o utilizzando la funzione subset:
subset(Food, subset=(Gender=="Male" & sports==1),
...
# sinistri per lesioni personali automobilistiche nei dati 
# assicurativi del pacchetto R.
...
# il subsetting?
length(AutoBi$CLMAGE[AutoBi$CLMAGE<20])
dim(subset(AutoBi, subset=CLMAGE<20, select=CLMAGE))
