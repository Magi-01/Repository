...
Analisi_Primaria(House_prices[ ,House_prices_numeric_colnames])
str(House_prices[,House_prices_numeric_colnames])
```
...
## Rimozione di outlier LotFrontage
```{r, warning = F}
...
## Rimozione di outlier per LotArea
```{r, warning = F}
...
densita alineati alla retta qq della norma.
Da notare che si butano il 50% dei dat
...
## Rimozione di oultier per MasVnrArea
```{r, warning = F}
...
## Rimozione di outlier per BsmtFinSF1
```{r, warning = F}
...
## Rimozione di outlier per BsmtFinSF2
```{r, warning = F}
rm_outlier_data = rm_oultiers(rm_0_data,.95)
...
## Rimozione di outlier per BsmtUnfSF
```{r, warning = F}
rm_outlier_data = rm_oultiers(rm_0_data,.95)
...
## Rimozione di 0 per TotalBsmtSF
```{r, warning = F}
...
## Rimozione di outlier per TotalBsmtSF
```{r, warning = F}
rm_outlier_data = rm_oultiers(rm_0_data,.95)
...
## Rimozione di outlier per X1stFlrSF
```{r, warning = F}
...
## Rimozione di outlier per X2ndFlrSF
```{r, warning = F}
rm_outlier_data = rm_oultiers(rm_0_data,.90)
...
## Rimozione di 0 per LowQualFinSF
```{r, warning = F}
rm_0_data = rm_0(na.omit(House_prices[,"LowQualFinSF"]))
print(paste("Ratio: ",length(rm_0_data)/length(na.omit(House_prices[,"LowQualFinSF"]))))
...
## Rimozione di outlier per GrLivArea
```{r, warning = F}
...
## Rimozione di outlier per GarageArea
```{r, warning = F}
rm_outlier_data = rm_oultiers(na.omit(House_prices[,"GarageArea"]),.90)
...
## Rimozione di outlier per WoodDeckSF
```{r, warning = F}
rm_outlier_data = rm_oultiers(rm_0_data,.90)
...
## Rimozione di 0 per OpenPorchSF
```{r, warning = F}
...
## Rimozione di outlier per OpenPorchSF
```{r, warning = F}
rm_outlier_data = rm_oultiers(rm_0_data,.90)
...
## Rimozione di 0 per EnclosedPorch
```{r, warning = F}
...
## Rimozione di outlier per EnclosedPorch
```{r, warning = F}
rm_outlier_data = rm_oultiers(rm_0_data,.95)
...
## Rimozione di 0 per X3SsnPorch
```{r, warning = F}
rm_0_data = rm_0(na.omit(House_prices[,"X3SsnPorch"]))
print(paste("Ratio: ",length(rm_0_data)/length(na.omit(House_prices[,"X3SsnPorch"]))))
...
e con maggior densita alineati alla retta qq della norma.
Gli Portico per tre stagioni costruiti sono cosi piccoli 
...
## Rimozione di 0 per ScreenPorch
```{r, warning = F}
...
## Rimozione di outlier per ScreenPorch
```{r, warning = F}
rm_outlier_data = rm_oultiers(rm_0_data,.95)
...
## Rimozione di 0 outlier per PoolArea
```{r, warning = F}
rm_0_data = rm_0(na.omit(House_prices[,"PoolArea"]))
print(paste("Ration: ", length(rm_0_data)/length(na.omit(House_prices[,"PoolArea"]))))
...
## Rimozione di 0 per MiscVal
```{r, warning = F}
rm_0_data = rm_0(na.omit(House_prices[,"MiscVal"]))
print(paste("Ratio: ", length(rm_0_data)/length(na.omit(House_prices[,"MiscVal"]))))
...
sono comprese tra 437.5 e 887.5 , 
essendo rispettivamente il 1° e il 3° quantile.
Dato la variabilita dei dati e la mancanza di coerenza, dovuta alla mancanza di dati (~=3.56%) dei dati totali, allora si potrebbe trascurare
...
## Rimozione di outlier per SalePrice
```{r, warning = F}
rm_outlier_data = rm_oultiers(na.omit(House_prices[,"SalePrice"]),.90)
